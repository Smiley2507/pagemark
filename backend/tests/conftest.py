...
async def test_org(db: AsyncSession, test_user: User) -> Organization:
    """Create a test organization with test user as admin."""
    org = Organization(
...
@pytest.fixture
async def client(db_engine, test_user: User) -> AsyncClient:
...
@pytest.fixture
async def other_user_client(db_engine, other_user: User) -> AsyncClient:

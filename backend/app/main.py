...
BUILTIN_TEMPLATES = [
    {
        ...
        "sections_json": [
            {
                "heading": "Overview",
                "description": "API overview, base URL, accepted content types, and versioning strategy.",
                "guidance": "State the base URL(s), supported content types (JSON, form-data, etc.), and how API versioning works (path prefix, header, or query param). Mention any global request ID or tracing headers the project supports.",
                "expected_sources": ["README.md", "docs/", "openapi.json", "swagger.json"]
            },
            {
                "heading": "Authentication",
                "description": "Every auth method the API accepts: API keys, OAuth2 flows, JWTs, basic auth.",
                "guidance": "List every authentication method the project supports. For each method, show how to include it in a request (header, cookie, query param). Call out any that are deprecated or only available on certain plans/environments. Include an example of obtaining and using a token for OAuth2 flows.",
                "expected_sources": ["auth/", "middleware/auth*", "config/auth*"]
            },
            {
                "heading": "Endpoints",
                "description": "Complete endpoint reference grouped by resource or domain.",
                "guidance": "Group endpoints by resource domain (Users, Orders, etc.). For each endpoint show: HTTP method, full path path with path parameters in curly braces, required and optional query/body parameters with types and descriptions, at least one example request and response. Reference the source file where the route handler is defined so readers can navigate the codebase.",
                "expected_sources": ["routes/", "controllers/", "api/", "handlers/"]
            },
            {
                "heading": "Request & Response Formats",
                "description": "Shared payload schemas and data types referenced by multiple endpoints.",
                "guidance": "Document reusable objects and enums that appear in multiple endpoints. Show the full schema with field names, types, constraints (required, nullable, min/max, format), and examples. Omit fields that are internal or server-only. Link back to endpoint sections that use each schema.",
                "expected_sources": ["models/", "schemas/", "types/", "serializers/"]
            },
            {
                "heading": "Error Codes",
                "description": "Standard error response format and per-endpoint error scenarios.",
                "guidance": "Describe the standard error response structure (status code, error code, message, request ID). List every error code the API can return, the HTTP status it maps to, and what the caller should do to resolve it. Include both global errors (rate limited, unauthorized) and per-endpoint errors.",
                "expected_sources": ["middleware/error*", "exceptions/", "errors/"]
            },
            {
                "heading": "Rate Limiting",
                "description": "Request quotas, reset intervals, and retry-after behavior.",
                "guidance": "Document the rate limit window, maximum requests per window, and which HTTP headers expose current status (X-RateLimit-Remaining, Retry-After, etc.). Describe what happens when a caller exceeds the limit and how to handle 429 responses. Note any per-endpoint or per-user tier differences in rate limits.",
                "expected_sources": ["middleware/ratelimit*", "config/", "deployment/"]
            },
        ],
    },
    {
        ...
        "sections_json": [
            {
                ...
                "description": "Installation command, minimum required runtime version, and a working 'hello world' example.",
                "guidance": "Show the exact install command for each supported package manager (npm, pip, cargo, etc.). State the minimum language runtime version. Provide a complete, runnable snippet that does something visible — this should be the simplest possible usage. Include import/require statements.",
                "expected_sources": ["README.md", "setup.py", "package.json", "Cargo.toml", "examples/"]
            },
            {
                "heading": "Configuration",
                "description": "Every configuration option the SDK accepts: constructor args, env vars, config files.",
                "guidance": "List every configuration parameter with its name, type, default value, and what it controls. Show code examples for the most common configurations: setting API keys, changing the base URL, configuring timeouts, enabling logging. Group options by concern (auth, networking, logging).",
                "expected_sources": ["src/config*", "src/client*", "__init__.py", "index.ts"]
            },
            {
                "heading": "Core Concepts",
                "description": "Key classes, interfaces, and patterns the developer must understand to use the SDK.",
                "guidance": "Document the main class hierarchy and how the pieces fit together. Explain the client lifecycle (create, configure, use, dispose). Cover the core patterns the SDK uses: builders, fluent APIs, callbacks/promises, streaming, pagination. Each concept must link to source evidence showing the actual type or interface.",
                "expected_sources": ["src/client*", "src/core*", "types/", "interfaces/"]
            },
            {
                "heading": "Code Examples",
                "description": "Reusable code snippets for the most common integration tasks.",
                "guidance": "Provide 3-5 complete examples covering the most common use cases: creating a resource, reading/listing, updating, deleting, and error handling. Each example should be self-contained with imports, setup, the core logic, and cleanup. Use the project's own test files or examples directory as source evidence for realistic patterns.",
                "expected_sources": ["tests/", "examples/", "samples/"]
            },
            {
                "heading": "Troubleshooting",
                "description": "Common errors, their causes, and how to resolve them.",
                "guidance": "List the most common errors developers encounter when integrating this SDK. For each error, show the error message, explain what causes it (wrong import, missing config, auth failure, version mismatch), and provide the fix. Check the project's issue tracker for frequently reported integration problems.",
                "expected_sources": ["README.md", "CONTRIBUTING.md", "issues/", "CHANGELOG.md"]
            },
        ],
    },
    {
        ...
        "sections_json": [
            {
                ...
                "description": "What the product does, who it is for, and what problem it solves.",
                "guidance": "Describe the product in one sentence. State the target user. List the key capabilities the manual covers. If there are system requirements (OS, browser, runtime), list them here. Link to the getting started section.",
                "expected_sources": ["README.md", "docs/intro*", "landing page assets"]
            },
            {
                "heading": "Getting Started",
                "description": "First-run experience: installation or sign-up, initial setup, and first task.",
                "guidance": "Walk through the first-run experience step by step. Cover: creating an account or installing the package, initial configuration or profile setup, and completing a simple first task that demonstrates value. Screenshots or terminal output should be referenced where available.",
                "expected_sources": ["README.md", "quickstart*", "docs/getting-started*"]
            },
            {
                "heading": "Features Guide",
                "description": "Walkthroughs of each major feature, organized by user workflow.",
                "guidance": "For each major feature, answer: 'What can I do here?' and 'How do I do it?' Use step-by-step numbered instructions. Reference exact UI labels, button names, and menu paths. Each walkthrough should be self-contained so users can jump to the feature they need.",
                "expected_sources": ["src/components/", "src/pages/", "docs/features*"]
            },
            {
                "heading": "Settings & Preferences",
                "description": "Every configurable option: UI settings, CLI flags, environment variables, config files.",
                "guidance": "List every setting the user can change, grouped by category (General, Notifications, Security, etc.). For each setting: name, possible values, default, and what it affects. Show where in the UI or which CLI flag controls it.",
                "expected_sources": ["src/settings*", "src/config*", "docs/settings*"]
            },
            {
                "heading": "FAQ",
                "description": "Answers to the most common questions users ask.",
                "guidance": "Answer the top 10-15 questions users commonly ask. Check the project's issue tracker, support tickets, or community forums for real user questions. If source analysis reveals a feature that is commonly misunderstood, include it here.",
                "expected_sources": ["README.md", "issues/", "discussions/", "docs/faq*"]
            },
            {
                "heading": "Support",
                "description": "How to get help: documentation links, community resources, bug reporting, contact information.",
                "guidance": "List every support channel: official documentation, community forum or chat, GitHub issues for bugs, email or contact form for private matters. Include response time expectations if disclosed. Provide the project's health status page URL if one exists.",
                "expected_sources": ["README.md", "CONTRIBUTING.md", ".github/"]
            },
        ],
    },
    {
        ...
        "sections_json": [
            {
                ...
                "description": "High-level description of what the system does, its main subsystems, and how they relate.",
                "guidance": "Start with a one-paragraph summary of the system's purpose. Then describe each major subsystem identified from the source directory structure. List the languages and frameworks used in each subsystem. Include a simple ASCII architecture diagram or describe the boundaries between subsystems.",
                "expected_sources": ["README.md", "src/ directory structure", "docs/architecture*"]
            },
            {
                "heading": "Component Diagram",
                "description": "Service map, process boundaries, and dependency relationships between components.",
                "guidance": "Map every significant directory to its role (API server, worker, database layer, frontend, CLI). Show which components depend on which. If the project is a monorepo, document each package and its dependencies. Reference the actual directory paths for each component so readers can navigate the codebase.",
                "expected_sources": ["src/", "packages/", "services/", "docker-compose*", "Cargo.toml"]
            },
            {
                "heading": "Data Flow",
                "description": "Request lifecycle through the system: entry, processing, storage, response.",
                "guidance": "Trace the path of a typical request or event through the system: from HTTP handler or CLI entry point through business logic to storage and back. For each step, name the module, function, or class involved and reference its source path. Include error paths: what happens when a step fails?",
                "expected_sources": ["routes/", "controllers/", "services/", "handlers/", "middleware/"]
            },
            {
                "heading": "Technology Stack",
                "description": "Languages, frameworks, databases, infrastructure, and rationale for each choice.",
                "guidance": "List every technology the project uses, categorized by layer (language, framework, database, infrastructure). For each, cite the evidence file (package.json reveals Express, Dockerfile reveals base image). Include version numbers where meaningful. Note any technologies that are being migrated away from or evaluated.",
                "expected_sources": ["package.json", "Cargo.toml", "pyproject.toml", "Dockerfile*", "docker-compose*"]
            },
            {
                "heading": "Deployment Architecture",
                "description": "How the system is built, deployed, hosted, and monitored.",
                "guidance": "Describe the deployment pipeline from source to production: CI system, build process, artifact storage, deployment targets, and monitoring. Reference actual deployment configs (Dockerfiles, Helm charts, CI YAML files). If the project does not contain deployment configs, note that deployment is external and describe any documented requirements.",
                "expected_sources": [".github/", ".gitlab-ci*", "Dockerfile*", "helm/", "k8s/", "deploy/"]
            },
        ],
    },
    {
        ...
        "sections_json": [
            {
                "heading": "Upgrade Overview",
                "description": "Summary of what changed between versions: intended audience, upgrade impact, estimated effort.",
                "guidance": "State the from-version and to-version. List each major change category and whether it requires code changes, config changes, or is a no-op migration. Give an estimated total effort (minutes to hours). Link to the relevant CHANGELOG or release notes.",
                "expected_sources": ["CHANGELOG.md", "RELEASES.md", "git tag --list"]
            },
            {
                "heading": "Configuration & Dependencies",
                "description": "Changes to configuration files, environment variables, and dependency versions.",
                "guidance": "List every config key, env var, and dependency that changed. For each: old key/name, new key/name, default change, and migration steps. Include the minimum supported version for each changed dependency. Use side-by-side config file diffs where possible.",
                "expected_sources": ["CHANGELOG.md", "config/", ".env.example", "package.json", "pyproject.toml"]
            },
            {
                "heading": "API & Breaking Changes",
                "description": "Every breaking change to the public API surface with before/after migration examples.",
                "guidance": "List every breaking API change. For each: the old function/class/endpoint signature, the new signature, why it changed, and the exact code change required. Use side-by-side code blocks labeled 'Before' and 'After'. Include both the error a user would see and the fix.",
                "expected_sources": ["CHANGELOG.md", "UPGRADING.md"]
            },
            {
                "heading": "Deprecations & Removals",
                "description": "Features deprecated in earlier versions that are now removed, with migration paths.",
                "guidance": "List every feature that was previously deprecated and is now removed. For each: when it was deprecated, what replaces it, and the final migration window. If a feature was removed without a replacement, explain the rationale and alternative approaches.",
                "expected_sources": ["CHANGELOG.md", "DEPRECATIONS.md"]
            },
        ],
    },
    {
        ...
        "sections_json": [
            {
                ...
                "description": "How to install the CLI tool and perform initial setup.",
                "guidance": "Show install commands for each supported platform (npm, pip, brew, cargo, binary download). Include any post-install steps like initializing a config file or setting environment variables.",
                "expected_sources": ["README.md", "install.sh", "setup.py", "package.json"]
            },
            {
                "heading": "Global Flags",
                "description": "Flags available on every command: output format, verbosity, config path, etc.",
                "guidance": "List every global flag in a table: flag name, short alias, type, default, description. Include standard flags like --help, --version, --verbose, --quiet, --config, --output-format. Note any environment variable equivalents.",
                "expected_sources": ["src/cli*", "src/main*", "cli/root*"]
            },
            {
                "heading": "Commands",
                "description": "Complete command reference organized by command hierarchy.",
                "guidance": "Organize commands hierarchically. For each command, show: the full invocation syntax, a description of what it does, a table of its flags, its positional arguments, and 1-3 examples with realistic inputs and outputs. Group related subcommands.",
                "expected_sources": ["src/commands/", "cli/", "src/subcommands/"]
            },
            {
                "heading": "Exit Codes & Errors",
                "description": "All exit codes the CLI produces and common error resolutions.",
                "guidance": "List every exit code (except 0 for success) with its numeric value, meaning, and common causes. Include the error message the user sees and steps to resolve each error.",
        ...
        "sections_json": [
            {
                ...
                "description": "Step-by-step instructions to set up a local development environment from a fresh clone.",
                "guidance": "Start from `git clone`. List every prerequisite: runtime version, package manager, database, external services. Show exact commands for each step. Include instructions for running the project locally (dev server, watcher). Verify these steps against the project's actual setup scripts and Makefile.",
                "expected_sources": ["README.md", "CONTRIBUTING.md", "Makefile", "Dockerfile*", "setup.py"]
            },
            {
                "heading": "Coding Standards",
                "description": "Linting, formatting, type checking, and naming conventions enforced by the project.",
                "guidance": "List every code quality tool and the commands to run it. Reference the project's actual config files (.eslintrc, .prettierrc, pyproject.toml, rustfmt.toml). Document naming conventions and file organization patterns found in the source structure.",
                "expected_sources": [".eslintrc*", ".prettierrc*", "pyproject.toml", "rustfmt.toml", "Makefile"]
            },
            {
                "heading": "Testing",
                "description": "How to write and run tests: test framework, directory structure, fixtures, required services.",
                "guidance": "State the test framework and command to run all tests. Explain the test directory structure. Document how to write a new test: where to put it, how to use fixtures, how to mock external services. Include instructions for running a subset of tests during development.",
                "expected_sources": ["tests/", "Makefile", "package.json scripts", "pyproject.toml"]
            },
            {
                "heading": "Pull Request Workflow",
                "description": "Branch naming, commit conventions, PR template, CI checks, and review process.",
                "guidance": "Document the expected branch naming scheme (feature/, fix/, etc.). State the commit message convention (conventional commits? linear history?). Walk through the CI pipeline: which checks run on push vs PR. Explain the review criteria and approval process.",
                "expected_sources": ["CONTRIBUTING.md", ".github/", "PULL_REQUEST_TEMPLATE*", "git log"]
            },
            {
                "heading": "Documentation",
                "description": "How to update documentation alongside code changes.",
                "guidance": "Document where different types of docs live (README, inline code docs, API docs, user-facing docs). Explain the expectation for doc changes alongside code changes. Reference any documentation generation tools (Sphinx, Typedoc, rustdoc).",
                "expected_sources": ["README.md", "CONTRIBUTING.md", "docs/", "pyproject.toml", "package.json"]
            },
        ],
    },
    {
        ...
        "sections_json": [
            {
                ...
                "description": "Alphabetical table of every configuration option with type, default, and setting location.",
                "guidance": "Provide a single comprehensive table with columns: Setting Name, Type, Default, Config File Key, Env Var, CLI Flag. Alphabetical order within domain groups. This section is the landing page for operators who just need to look up a setting.",
                "expected_sources": ["src/config*", "src/settings*", ".env.example", "config/"]
            },
            {
                "heading": "Environment Variables",
                "description": "Every environment variable the project accepts, with validation and examples.",
                "guidance": "List every environment variable the project reads, organized by prefix or domain. Include: full variable name, required or optional, type, default value, validation constraints, and an example value. Note any that are sensitive (secrets, tokens) and must be handled securely.",
                "expected_sources": [".env.example", "src/config*", "docker-compose*", "deploy/"]
            },
            {
                "heading": "Configuration File",
                "description": "Full config file schema with example files for common deployment scenarios.",
                "guidance": "Document the complete config file schema. Show the file format (YAML, TOML, JSON, INI). Provide annotated example files for development, production, and testing scenarios. Document any hierarchical or nested configuration structure.",
                "expected_sources": ["config/", "src/config*", ".env.example", "pyproject.toml"]
            },
            {
                "heading": "Runtime Settings",
                "description": "Settings that can be changed at runtime without restarting the process.",
                "guidance": "List every setting that supports hot-reload or runtime changes. For each: the mechanism to change it (API endpoint, signal, admin panel), the scope of the change (per-request, per-session, global), and how to verify the change took effect.",
                "expected_sources": ["src/config*", "routes/admin*", "middleware/"]
            },
        ],
    },
]
